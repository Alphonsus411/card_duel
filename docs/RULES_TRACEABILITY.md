...
| R-COMPAT-019-REPLAY | Replay semántico 0.19 | No es una regla normativa. | No aplica: es compatibilidad técnica. | Compatibilidad temporal. | La deserialización v2 no basta para reproducir comandos cuyo significado cambió en 0.20. El replay activa sólo durante su ejecución un modo 0.19 que acepta Drenaje fuera de Fase de Efectos, Desafío en Combate y la elegibilidad histórica; después restaura modo normal. Los cuatro fixtures se generaron con el commit histórico documentado en `tests/artifacts/0.19.0/README.md`; hashes, observables, restauración ante error y rechazo de versiones desconocidas están cubiertos por `test_replay_legacy_019.py`. Es un puente de vida limitada, no una vía para partidas nuevas, y deberá retirarse mediante una decisión de compatibilidad versionada. | ya cumple |
