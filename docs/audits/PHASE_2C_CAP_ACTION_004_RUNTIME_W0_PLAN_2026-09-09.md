...
| `W0-GATE-CAS` | Persistencia del registro dentro del único snapshot sometido al CAS existente. | **PARCIAL (gate global)** — **CAS de persistencia del snapshot W0 implementado y probado** en InMemory y SQLite: exactamente un ganador, un `VersionConflict`, un incremento y persistencia íntegra de un candidato. **CAS del cierre público y ausencia de evento fantasma pendientes de W1**, porque requieren comandos/eventos lifecycle y publicación application/service que W0 no introduce ni simula. | Tests de carrera de persistencia W0 en ambos stores; cierre público concurrente y observables post-CAS fuera de alcance hasta W1. |
