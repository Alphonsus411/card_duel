...
|---|---|---|---|---|---|---|
| R-03B.3-DECK | Construcción de mazos | Reglas básicas 1–2, p. 5: igualdad y mínimo de 50 puntos; no fija cantidad de cartas. | Física 2 / interna 1: 40–60 cartas; el presupuesto de puntos es contradictorio (`N-POINTS-01`). | B — formato de mazo, con extremo E en puntos. | `mythic_deck_policy` aplica 40–60 sólo cuando la aplicación selecciona el formato y deja el presupuesto en `None`. | **implementado sin resolver `N-POINTS-01`**: un presupuesto sólo puede llegar por configuración explícita. |
| R-03B.3-COPIES | Copias máximas | P. 5: no establece un máximo de copias verificable. | Física 2 / interna 1: hasta cinco no Legendarias y cuatro Legendarias. | B — formato de mazo. | La política Mística cuenta por `card_id` y consulta el `rank` canónico de cada definición. | **implementado en el perfil Místico**: Clásico no inventa límites generales y permite configurarlos. |
