...
| `CAP-ACTION-004` — Decisión pendiente autorizada | PARTIAL | CAP-ACTION-002, CAP-PRIVACY-001 | CAP-SECRET-002; CAP-TIME-002 | **CRITICAL** — doble resolución, autorización obsoleta, fuga de audiencia o replay divergente | W1.3 acredita historia total tipada, lifecycle interno reproducible `None → PENDING → CLOSED → None`, replay v3 y snapshot v4 fail-closed; apertura pública, bloqueo por familia y audiencias adicionales siguen pendientes |
...
| `CAP-ZONE-002` — Zonas base y transiciones | SUPPORTED | CAP-ZONE-001, CAP-ACTION-003 | CAP-ZONE-003; CAP-TIME-001; CAP-TIME-005; CAP-SEARCH-001; CAP-ATTACH-001 | **MEDIUM** — contrato local incoherente o UX/replay divergente | `domain/enums.py`, `domain/models.py`, `engine/zones.py`, `engine/game.py`, `engine/stack.py`, `persistence/` |
...
## Planificación de `CAP-TIME-005`

* **Descripción y límite:** orquesta la creación de participantes, mazos y manos iniciales, conserva el estado `SETUP`, sigue las decisiones obligatorias y prepara la transición cuando todas cierran. No elige jugador inicial, no concede prioridad y no entra en la primera fase.
* **Referencias normativas:** `N-PHASE-01` para el lifecycle de preparación; `N-PHASE-02` exclusivamente para la condición de cierre de sus decisiones. La prioridad inicial continúa aislada en `CAP-TIME-001`.
* **Estado y bloqueo:** `MISSING`; no puede promoverse a `SUPPORTED`/`CLOSED` sin evidencia del contrato completo. Hasta entonces el readiness de `N-PHASE-02` sigue `BLOCKED`.
* **Prerequisites y dependientes:** sólo `CAP-ZONE-002` y `CAP-PRIVACY-001`; su dependiente directo es `CAP-TIME-002`, que ya no depende de `CAP-TIME-001`. La decisión pendiente se mantiene dentro del contrato hasta que exista una abstracción universal justificada.
* **Superficies futuras:** `domain/enums.py`, `domain/models.py`, `engine/commands.py`, `engine/game.py`, `engine/phases.py`, `application.py`, `service.py`, `persistence/` y `storage/`.
* **Riesgo, prioridad y wave:** **CRITICAL**, **P0**, **W1**. Una implementación parcial puede saltar decisiones, filtrar manos, divergir en replay o comenzar fases/prioridad prematuramente.
* **Impacto de corpus:** regla universal sobre las 431 entradas; afecta indirectamente 386 identidades y 45 variantes, sin atribuir ni sumar impacto por carta.
...
`CAP-ACTION-004` se limita a la infraestructura universal de una decisión pendiente autorizada: `decision_id`, elector, audiencia, conjunto de opciones opacas, estado pendiente/resuelto, autorización, rechazo o invalidación por versión obsoleta, resolución exactamente una vez, persistencia, snapshot, replay y CAS. **Excluye expresamente** candidatos de cartas, cardinalidad, ordenación, selección compuesta, *simultaneous reveal* y semántica de búsquedas; esos contratos permanecen en `CAP-SECRET-002` u otras capabilities especializadas. W1.3 añade una única historia total tipada de comandos y transiciones `OPENED/CLOSED/CONSUMED`, reproduce el slot `None → PENDING → CLOSED → None` mediante replay v3 y conserva `CLOSED` hasta un consumo interno explícito. Snapshot v4 persiste esa historia y marca como incompleto, de forma *fail-closed*, cualquier prefijo v3 que contenga un lifecycle no demostrable. Permanecen fuera la apertura pública arbitraria, el bloqueo selectivo por `semantic_family`, decisiones simultáneas y audiencias adicionales; por ello permanece `PARTIAL / READY`, no inicia W1.4 y no desbloquea `CAP-TIME-002`.
...
| `CAP-TIME-001` | Existen `start_match` y campos de prioridad reutilizables. | Falta autoridad normativa y recorrido persistible/autorizado para seleccionar quién empieza y concederle prioridad. Este bloqueo impide comenzar la partida o entrar en la primera fase, pero no ejecutar ni persistir mulligan dentro de `SETUP`. |
| `CAP-TIME-005` | No se acredita todavía un recorrido único que cree participantes/mazos/manos, mantenga `SETUP`, registre todas las decisiones obligatorias y prepare la transición cerrada. | Implementar y demostrar el contrato completo en modelo, comandos, motor, aplicación/servicio, persistencia/replay y proyección, sin conceder prioridad ni entrar en la primera fase. `N-PHASE-01` rige la preparación y `N-PHASE-02` sólo el cierre de decisiones; mientras esta capability siga abierta, `N-PHASE-02` permanece `BLOCKED`. |
| `CAP-TIME-002` | El mulligan interno conserva un contador persistido, se limita al setup, aplica tamaños decrecientes 5→1 y reshuffle determinista, emite evento, entra en el snapshot básico y permite reconstrucción de replay. | Comando público KEEP/REPLACE, pending decision, legal-action parity, CAS, capas application/service, privacidad específica, lifecycle completo de setup y compatibilidad versionada. |
...
La clasificación es uniforme: `MISSING` significa ausencia de cualquier subconjunto ejecutable demostrable; `PARTIAL`, que existe un subconjunto real pero no el contrato completo; y `SUPPORTED`, que está probado el recorrido público, persistente, replayable y de servicio completo. Las `BLOCKED` requieren primero autoridad normativa; implementar una interpretación local no cambia su estado.
