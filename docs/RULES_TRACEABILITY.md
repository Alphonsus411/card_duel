...
|---|---|---|---|---|---|---|
| R-03B.3-DECK | Construcción de mazos | Reglas básicas 1–2, p. 5: igualdad y mínimo de 50 puntos; no fija cantidad de cartas. | Física 2 / interna 1: 40–60 cartas; el presupuesto de puntos es contradictorio (`N-POINTS-01`). | B — formato de mazo, con extremo E en puntos. | `mythic_deck_policy` y `classic_deck_policy` aplican 40–60 cuando la aplicación selecciona un formato y dejan el presupuesto en `None`. | **implementado sin resolver `N-POINTS-01`**: la lectura directa sitúa 40–60 antes de separar los formatos; un presupuesto sólo puede llegar por configuración explícita. |
| R-03B.3-COPIES | Copias máximas | P. 5: no establece un máximo de copias verificable. | Física 2 / interna 1: hasta cinco no Legendarias y cuatro Legendarias. | B — formato de mazo. | Ambas políticas cuentan por `card_id` y consultan el `rank` canónico de cada definición. | **implementado en ambos perfiles**: la lectura directa presenta 5/4 como ajuste general antes de los apartados Clásico y Mística. |
| R-03B.3-ZERO | Cartas de coste cero | Reglas básicas 1 y 3, p. 5: el coste interviene en el ajuste de mazo y las cartas se pagan para jugarse. | Física 2 / interna 1: Clásico limita las de coste cero; Mística fija costes 5–50 para sus cartas. | B — restricción de formato. | Las fábricas Clásico/Mística aplican sus límites de coste cero y el intervalo Mítico sin cambiar `CardDefinition`. | **implementado por formato**: no existe una prohibición universal. |
| R-03B.3-FORMATS | Formatos Clásico y Mística | Sin esos formatos en las reglas básicas verificadas, pp. 5–8. | Física 2 / interna 1 y física 3 / interna 2: ediciones admitidas y circuito de cada formato. | B — formato de mazo. | `DeckConstructionPolicy` ofrece perfiles opcionales con legalidad de colección inyectada antes de crear la partida. | **implementado como política optativa**: `RuleSet` y el comportamiento sin política permanecen intactos. |
