...
Fecha de revisión: 2026-09-07. Este registro depende del documento maestro
...
| `N-MULLIGAN-01` | `OPEN/BLOCKED` (`AMBIGUOUS`) | Base fija el contenido cuantitativo de `N-PHASE-02`, pero no define el protocolo observable y multijugador necesario para ejecutarlo sin decisiones adicionales. Véase el desglose estable inferior. | Implementar únicamente los extremos expresamente autorizados; cada dimensión abierta debe ser un blocker o una exclusión de slice, nunca un default técnico con efecto normativo. Esta entrada complementa y no reemplaza ni reinterpreta `N-PHASE-02`. | Texto oficial que cierre individualmente cada dimensión abierta, o exclusión explícita de esa dimensión en un slice que pueda completarse sin resolverla. |
...
### Desglose estable de `N-MULLIGAN-01`

Este desglose separa lo que sí dice la autoridad de las decisiones que siguen
abiertas. Sus subidentificadores son granulares y deben citarse por separado en
roadmaps, blockers y futuras resoluciones.

| Sub-ID estable | Clasificación | Extremo conservado |
|---|---|---|
| `N-MULLIGAN-01.AUTH-HAND` | `AUTORIZADO` | La mano inicial es de seis cartas. |
| `N-MULLIGAN-01.AUTH-STEPS` | `AUTORIZADO` | Los reemplazos decrecen exactamente por tamaños 5 → 4 → 3 → 2 → 1. |
| `N-MULLIGAN-01.AUTH-STOP` | `AUTORIZADO` | Después de recibir una mano de una carta no puede repetirse el mulligan. |
| `N-MULLIGAN-01.OPEN-ORDER` | `OPEN/BLOCKED` | No consta quién decide primero ni cómo se determina el orden de decisión. |
| `N-MULLIGAN-01.OPEN-MODE` | `OPEN/BLOCKED` | No consta si las decisiones son alternadas o simultáneas. |
| `N-MULLIGAN-01.OPEN-REVEAL` | `OPEN/BLOCKED` | No consta cuándo una decisión pasa a ser públicamente observable. |
| `N-MULLIGAN-01.OPEN-STARTER` | `OPEN/BLOCKED` | No consta si la identidad del jugador inicial debe preservarse durante todo el protocolo ni frente a sus decisiones. |
| `N-MULLIGAN-01.OPEN-3PLUS` | `OPEN/BLOCKED` | No consta la conducta del protocolo con tres o más participantes. |

`N-MULLIGAN-01` no cambia la autoridad, el estado ni el significado de
`N-PHASE-02`: sólo registra por separado los datos cuantitativos expresos y los
vacíos protocolarios. Tampoco autoriza a inferir orden, simultaneidad,
visibilidad, preservación del jugador inicial o semántica multijugador a partir
de una estructura de datos, una prueba o una conveniencia de implementación.
